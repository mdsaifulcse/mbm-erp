                    ...
                    <div class="search-result-div col-xs-12 col-sm-4 widget-container-col ui-sortable">
                        <div class="widget-box widget-color-green2  ui-sortable-handle" id="widget-box-6">
                            <div class="widget-header">
                                <a href="#" class="white search_executive" data-team="{{ $team->id }}">
                                    <h5 class="widget-title smaller">  {{ $team->name }} </h5></a>
                            </div>

                            <div class="widget-body">
                                <div class="widget-main padding-6">
                                    <div class="profile-info-row">
                                        <div class="profile-info-name" > Team Lead </div>

                                        <div class="profile-info-value" >
                                            <span>{{ $team->lead}}</span>
                                        </div>
                                    </div>
                                    <div class="profile-info-row search_executive" data-team="{{ $team->id }}" style="cursor:pointer;">
                                        <div class="profile-info-name" > Team Member </div>
                                    ...
                                    <table class="table table-hover table-bordered no-margin" style="text-align:right;">
                                        ...
                                        <tr class="search_order" data-team="{{ $team->id }}"  data-status="Incomplete" style="color:#d89807;">
                                            <td style="text-align:center;">On Process</td>
                                            <td>{{($team->order)-($team->del_order)}}</td>
                                            <td>{{($team->qty)-($team->del_qty)}}</td>
                                            <td colspan="2">{{($team->tfob)-($team->del_fob)}}</td>
                                        </tr>
                                        <tr class="search_order" data-team="{{ $team->id }}"  data-status="Completed" style="color:#2e8965;">
                                            <td style="text-align:center;">Completed</td>
                                            <td>{{ $team->del_order}}</td>
                                            <td>{{ $team->del_qty}}</td>
                                            <td colspan="2"> {{ $team->del_fob}}</td>
                                        </tr>
                                        <tr class="search_order" data-team="{{ $team->id }}" >
                                            <td style="text-align:center;"> Total </td>
                                            <td> {{ $team->order}} </td>
                                            <td> {{ $team->qty}}  </td>
                                            <td colspan="2"> {{ $team->tfob}} </td>
                                        </tr>
                                    </table>
