...
@section('main-content')
	<div class="main-content">
	    ...
	    <div class="panel">
	    	<div class="panel-heading">
	    		<form id="formReport" role="form" method="get" action="{{ url("hr/audit/fetch/maternity_leave") }}" >
                    @csrf
		    		<div class="row pl-0">
		    			<div class="col-sm-6">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body" style="padding-top: 0;background: #f1f1f1;">
	      	<div class="d-flex justify-content-center">
		        <div style="width: 850px;" class="maternity-content-result  bg-white p-3" id="maternity-content-result"></div>
	      	</div>
	      </div>
	...
	<!-- audit action -->
	<div class="modal fade apps-modal" id="maternityAudit" tabindex="-1" role="dialog" aria-labelledby="appsModalLabel" aria-hidden="true" data-backdrop="false" >
                ...
                <div class="row">
                    <div class="col-md-12 ml-auto mr-auto">
                    	...
                    	<div class="custom-control custom-switch custom-switch-icon custom-switch-color custom-control-inline mb-2">
                    	  ...
                    	  <input type="hidden" id="audit-leave-id" value="">
...
@push('js')
	<script type="text/javascript">
		...
		$(document).on('click','#benefits', function(){
			...
			$("#maternity-content-result").html(loaderContent)
			var id = $(this).data('id');
			$.ajax({
		        url: '{{ url('hr/preview/maternity-benefits') }}/'+id,
		        type: "GET",
		        success: function(response){
		            if(response !== 'error'){
		                $("#maternity-content-result").html(response);
		            }else{
		                $("#maternity-content-result").html('Nothing found!');
		            }
		        }
    		});
		});
		...
		$(document).on('click','#auditAction', function(){
			$('#maternityAudit').modal('show')
			$('#audit-leave-id').val($(this).data('id'));
		});



		$(document).on('click', '#audit-submit', function($q){
			let leave_id = $('#audit-leave-id').val(),
			    status = ($('#audit-checked').is(":checked")?1:2),
			    comment = $('#audit-comment').val()

			$(this).attr('disabled',true)
			$.ajax({
		        url: '{{ url('hr/audit/action/maternity_leave') }}',
		        type: "POST",
		        data:{
		        	'_token' : "{{ csrf_token() }}",
		        	'id': leave_id,
		        ...
		        success: function(response){
		            ...
		            if(response.status == 1){
		            	$('.action-'+response.hr_maternity_leave_id).html('<i class="las la-check-circle text-success" style="font-size: 16px"></i>')
		            }else{
		            	$('.action-'+response.hr_maternity_leave_id).html('<i class="las la-times-circle text-danger" style="font-size: 16px" id="crossReason" data-message="'+response.comment+'" data-date="'+response.date+'"></i>')
		            }
		            $('#audit-submit').attr('disabled',false)
		            $('#maternityAudit').modal('hide')
		            $('#audit-leave-id').val('')
