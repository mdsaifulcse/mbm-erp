...
@endphp
<a href='{{ url("hr/operation/filter-wise-incentive-sheet?$urldata&output=excel")}}' target="_blank" class="btn btn-sm btn-info hidden-print" id="excel" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excel Download" style="position: absolute; top: 20px; left: 80px;"><i class="fa fa-file-excel-o"></i></a>
...
<div id="bill-print">
    ...
    <form class="" role="form" id="billReport" method="get" action="#">
                                                        ...
                                                        @foreach($getBillLists[$list->as_id]->chunk(2) as $billLists)
                                                            ...
                                                            <p style="margin:0;padding:0" >
                                                                <span style="text-align: left; width: 65%; float: left;  white-space: wrap; {{ $dateList->pay_status ==0?'color:hotpink':'' }}" >
                                                                    @php
                                                                        $singDate = date('d-m-Y', strtotime($dateList->date));
                                                                    @endphp
                                                ...
                                                <td>
                                                    <div class="flex-content" style="display: block; height: 100%; border: 0;">
                                                        ...
                                                        @foreach($getBillLists[$list->as_id] as $dateList)
                                                            @if(isset($attendance[$list->as_id][$dateList->date]))
                                                            <p style="margin:0;padding:0" >
                                                                
                                                                <span style="width: 40%; text-align: left; white-space: wrap; float: left;" >
                                                                    <font > {{ $attendance[$list->as_id][$dateList->date]->in_time == null?'null':Custom::engToBnConvert(date('H:i',strtotime($attendance[$list->as_id][$dateList->date]->in_time))) }}</font>
                                                                </span>
                                                                ...
                                                                <span style="width: 40%; text-align: left; float: left; white-space: wrap;" >
                                                                    <font > {{ $attendance[$list->as_id][$dateList->date]->out_time == null?'null':Custom::engToBnConvert(date('H:i',strtotime($attendance[$list->as_id][$dateList->date]->out_time))) }}</font>
                                                                </span>
                                                            </p>
                                                            @endif
                                                        @endforeach
        ...
        @endif
        <br>
        @if(auth()->user()->canany(['Incentive Bonus Operation']))
        <div class="row">
  ...
  <div class="modal-dialog modal-lg right-modal-width" role="document" > 
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h5 class="modal-title right-modal-title text-center" id="modal-title-right-group"> প্রোডাকশন ইন্সেন্টিভ বোনাস বিতরণ </h5>
...
<script type="text/javascript">
    ...
    $('#billReport').on('submit', function(e) {
        ...
        // ajax call
        $.ajax({
            url: '{{ url("/hr/operation/review-incentive")}}',
            type: "POST",
            data: form.serialize(),
    ...
    $('#confirm-disbursed').on('click', function(e) {
        ...
        // ajax call
        $.ajax({
            url: '{{ url("/hr/operation/pay-incentive")}}',
            type: "POST",
            data: form.serialize(),
            success: function(response){
                // console.log(response)
