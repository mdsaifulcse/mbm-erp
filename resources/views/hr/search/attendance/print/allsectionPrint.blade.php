...
<div id="full_body_area">
	<div id="header_area">
		...
		<h3 style="margin: 5px 0px;">Area: {{$areaName}} | Department: {{$departmentName}} | Floor: {{$floorName}}</h3>
		...
		<table>
			...
			<tbody>
				...
				<tr>
					...
					@php
						unset($response[$section['hr_section_id']]['attendance']);
					@endphp
					@foreach($response[$section['hr_section_id']] as $k=>$single)
						@php
							if($k=='present') {
								$present += $single;
							}
