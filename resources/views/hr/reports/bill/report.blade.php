<div class="panel">
	<div class="panel-body pt-0">
		...
		@php
			...
			$modeUrl = url("hr/reports/bill-announcement-report?$urldata&export=excel");
		@endphp

		<a href='{{ $modeUrl }}' target="_blank" class="btn btn-sm btn-info hidden-print" id="excel" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excel Download" style="position: absolute; top: 21px; left: 72px;"><i class="fa fa-file-excel-o"></i></a>
        ...
        .table thead th {
			...
			@php
				...
				$btype = $input['bill_type'];
			@endphp
			
			<div class="top_summery_section">
				<div class="page-header">
						<div>
							<h4 style="margin:4px 10px; font-weight: bold; text-align: center;">@if($btype != '') {{ $billType[$btype] }} @endif Bill @if($input['report_format'] == 0) Details @else Summary @endif Report </h4>
						</div>
            <div class="row w-100 " style="">
            	<div style="width:26%; padding-left:15px; padding-right:15px; float:left;">
            		...
            		@if(isset($input['selected']))
            		<p class="subhead-left-text"> <b> Selected : </b> 
            				{{ $input['body'] }}
            		</p>
            		@endif
            	</div>
		            ...
		            <table class="table no-border f-14" border="0" style="width:100%;margin-bottom:0;font-size:13px;text-align:left"  cellpadding="5">
		            	<tr>
		            		<td>
					            ...
					            <p style="text-align: center; font-size: 13px;">Total Bill : {{ bn_money(round($summary->totalBill,2)) }} </p>
		            		</td>
		            	</tr>
		            </table>
            	</div>
	          ...
	          @if(count($employees) > 0)
	            @foreach($employees as $employee)
	            	...
	            	<tr>
		            	...
		            	<td>
		            		<a type="button" class="w-100 bill-details" title="{{ $employee->as_name }} - Bill Date" data-id="{{ $employee->as_id}}" data-ename="{{ $employee->as_name }}" data-paytype="{{ $input['pay_type']??'null' }}" data-unit="{{ $employee->as_unit_id }}" data-fromdate="{{ $input['from_date'] }}" data-todate="{{ $input['to_date'] }}">{{ $employee->bill_count }}</a>
		            	</td>
	            	</tr>
					...
					<table class="table table-bordered table-hover table-head" border="1" style="width:100%;border:1px solid #ccc;margin-bottom:0;font-size:14px;text-align:left" cellpadding="5">
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-right">
									{{ bn_money(round($groupSal->billAmount)) }}
								</td>
								...
								<td class="text-right">
									{{ bn_money(round($groupSal->billAmount - $groupSal->paidAmount)) }}
								</td>
							</tr>
							@endforeach
							 <tr>
								...
								<td class="text-right fwb" style="font-weight: bold">{{ bn_money(round($summary->totalBill)) }}</td>
								...
								<td class="text-right fwb" style="font-weight: bold">{{ bn_money(round($summary->totalBill - $summary->totalPaidAmount)) }}</td>
							</tr>
...
<script>
  ...
  var mainurl = '/hr/reports/bill-announcement-report?';
  ...
  function selectedGroup(e, body){
  	...
  	$("#modal-title-right-group").html(' '+body+' Bill Details');
   ...
   //  $('[data-toggle="popover"]').popover({
	  ...
	  //   content: function() {
	  //     return $.ajax({
		 //      url : "/hr/reports/bill-single-report",
		 //      data:{
		 //      	as_id: $(this).data('id'),
