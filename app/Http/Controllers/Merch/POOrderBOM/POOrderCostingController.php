...
class POOrderCostingController extends Controller
{
	...
	public function showForm(Request $request)
	{
		...
		$style_other_costing= MrPoBomOtherCosting::where('mr_style_stl_id', $id)
		->first();
		...
		*---------------------------------------------
		*/
		$special_operation = DB::table("mr_po_operation_n_cost AS oc")
		->select(
			"oc.*",
			"o.opr_name"
		)
		...
		*---------------------------------------------
		*/
		return view("merch.po_bom.po_order_costing_form", compact(
			"order",
			"samples",
			"operations",
			"machines",
	...
	public function editForm($po_id, $order_id, $clr_id)
	{
			...
			// Other Costs of style for showing
			$style_other_costing= MrPoBomOtherCosting::where('mr_style_stl_id', $id)->first();
		...
		*/
		...
		->leftJoin("mr_composition AS com", "com.id", "b.mr_composition_id")
		->leftJoin("mr_construction AS con", "con.id", "b.mr_construction_id")
		->where(['b.po_id' => $po_id, 'b.clr_id' => $clr_id])
		->leftJoin('mr_order_entry AS OE', 'OE.order_id', 'b.order_id')
		->orderBy("b.mr_material_category_mcat_id")
		->get();
		...
		*/
		...
		->leftJoin("mr_operation AS o", "o.opr_id", "=", "oc.mr_operation_opr_id")
		->where(['oc.po_id' => $po_id, 'oc.clr_id' => $clr_id])
		->where("oc.opr_type", 2)
		->get();
		...
		foreach ($special_operation as $spo)
		{
			...
			<td>
			<input type=\"hidden\" name=\"style_op_id[]\" value=\"$spo->mr_operation_opr_id\"/>
			<input type=\"hidden\" name=\"mr_po_opr_n_cost_id[]\" value=\"$spo->id\"/>
		...
		*---------------------------------------------
		*/
		$other_cost = MrPoBomOtherCosting::where(['po_id' => $po_id, 'clr_id' => $clr_id])->first(); 
		// dd($other_cost);
		if($other_cost!=null){
		...
		*---------------------------------------------
		*/
		$check_booking = MrPoBomCostingBooking::where(['po_id' => $po_id, 'clr_id' => $clr_id ])
		...
		$color_name = DB::table('mr_material_color')->where('clr_id', $clr_id)->value('clr_name');
			...
			return view("merch.po_bom.po_order_costing_edit", compact(
			"order",
			"samples",
			"operations",
			"machines",
	...
	public function updatePOCosting(Request $request)
	{
		// dd($request->all());
		$validator = Validator::make($request->all(), [
   	    	// mr_stl_bom_n_costing - update
		...
		try {
			...
			PurchaseOrder::where('po_id', $request->tmp_po_id)->update(['country_fob' => $request->agent_fob ]);
			// Store Style Bom and Costing
			if (is_array($request->id) && sizeof($request->id) > 0)
			{
   	    		// mr_stl_bom_n_costing - update				
   	    		for ($i=0; $i<sizeof($request->id); $i++)
				{
					...
					MrPoBomCostingBooking::where("id", $request->id[$i])->update($update);
				...
				if (is_array($request->style_op_id) && sizeof($request->style_op_id) > 0)
				{
					for ($i=0; $i<sizeof($request->style_op_id); $i++)
					{
				    	...
				    	//---------------------------------------
						DB::table("mr_po_operation_n_cost")
						->where("id", $request->mr_po_opr_n_cost_id[$i])
						->update($update);
	   	    	...
	   	    	// mr_stl_bom_other_costing - insert
				MrPoBomOtherCosting::where("id", $request->other_cost_id)
				->update([
				"mr_style_stl_id"  => $request->mr_style_stl_id,
				"testing_cost" => $request->testing_cost,
				"cm"           => $request->cm,
		  ...
		  //   			// update mr style table status = 2
		    	...
		    	*/
				DB::commit();
				return back()->with('success', 'Save / Update successful.');
			}
			else
			{
				return back()
