...
class OrderBreakDownControllerNew extends Controller
{
...
//        dd($id);
      try {
          ...
          $poList = DB::table('mr_purchase_order as a')
            ...
            $clrs = collect($poList)->pluck('clr_name','po_id');
            

            $sizeList = DB::table('mr_purchase_order as a')
                      ->select(
                        'a.po_id',
                        'a.mr_order_entry_order_id',
                        'a.po_no',
                        ...
                        'mc.clr_name',
                        'c.qty',
                        'd.mr_product_pallete_name'
                      )
                      ->leftJoin('mr_country as b','b.cnt_id','a.po_delivery_country')
                      ->leftJoin('mr_po_size_qty as c','c.po_id','a.po_id')
                      ->leftJoin('mr_product_size as d','d.id','c.mr_product_size_id')
                      ->leftJoin("mr_material_color AS mc", "mc.clr_id", "a.clr_id")
                      ->where('a.mr_order_entry_order_id',$id)->get();
         ...
         // dd($sizes);
          ...
          // po sub item qty amount
          $getPoId = collect($poList)->pluck('po_id');
          ...
          $totalPoQuantity = collect($uniqueSizeQty)->sum();
      	...
      	return view('merch.order_breakdown.order_breakdown_show_new',
              compact('order','boms','clrs','colors','uniqueSizeQty','totalPoQuantity','sizes','sizeList','care_label','filter','poSizeQtyList','poSizeQtyListC','poSizeQtyListS','poList', 'itemUnique', 'catCount','poSizeQtyListN','poSizeQtyListCN','poSizeQtyListSN','poColorQtyList','poColorQtyListC','poColorQtyListN','poColorQtyListCN'));
      } catch(\Exception $e) {
          return $e->getMessage();
      }
    }
