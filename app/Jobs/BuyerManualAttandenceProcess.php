...
class BuyerManualAttandenceProcess implements ShouldQueue
{
     ...
     */
     ...
     public $associate_id;
     public $unitId;
     public function __construct($intime, $associate_id, $unitId)
     {
         $this->intime = $intime;
     ...
     */
     public function handle()
     {
       ...
       $request = new Benefits;
        $date = explode(' ',$this->intime);
       $request->date = $date[0];
       $request->unit =$this->unitId;
        //dd($this->intime);exit;
       $employee = DB::table('hr_as_basic_info')->where('as_id',$this->associate_id)->where('as_status',1)->first();
       ...
       //foreach ($employees as $employee) {
         ...
         foreach ($templates as $template) {
           ...
           //dd($result['date']);exit;
               ...
               $this->generate_salary_day_wise($result,$request->associate,$request->buyer_template_id);
           ...
           if ($fetchUser->exists()) {
               ...
               for($i=1; $i<=$totalDays; $i++) {
                 ...
                 //$buyerTemplate->ot_hour = 2;
                   ...
                   $attendance['date'] = $startDay;
                   $attendance['in_time'] = !empty($data->in_time)?$data->in_time:null;
                   $attendance['out_time'] = !empty($data->out_time)?$data->out_time:null;
                   $attendance['overtime_time'] = (($info->as_ot==1)? $data->overtime_time:"");

                   if($data->leaves== true){
                       $attendance['present_status']=$data->leave_type." Leave";
                   } else if($data->holidays>=0) {
                       if($data->holidays==1) {
                         $attendance['present_status']="Weekend(General)";
                           $total_attends++;
                       }
                       else if($data->holidays==2){
                         $attendance['present_status']="Weekend(OT)";
                           $total_attends++;
                       }
                       else if($data->holidays==0)
                         $attendance['present_status']=$data->holiday_comment;
                   } else {
                       if($data->attends){
                         if(strtotime($attendance['in_time']) >(strtotime($data->shift_start_time)+180)){
                             $attendance['present_status']="P(late)";
                             // $data[] = $employee; //'Present (Late)';
                         } else {
                           $attendance['present_status']="P";
