...
class BuyerAttandenceInsertProcess implements ShouldQueue
{
     ...
     */
     public $data;
     public $unitId;
    public function __construct($data,$unitId)
    {
        $this->data = $data;
       $this->unitId = $unitId;
    }
          ...
          foreach ($results as $associate => $result) {
              foreach ($result as $templateId => $attDatas) {
                    ...
                    foreach ($attDatas as $key => $attData) {
                      $exists = DB::table('hr_buyer_att')->where('as_id',$associate)->first();
                      if(empty($exists)){
                        ...
                        DB::table('hr_buyer_att_template')->insert([
                          ...
                          'in_time' => $attData['date'].' '.$attData['in_time'],
                          'out_time' => $attData['date'].' '.$attData['out_time'],
                          'late_status' => $attData['present_status'] == "P(late)"?1:0,
                          'present_status' => $attData['present_status'],
                          'ot_hour'=> isset($attData['overtime_time'])?$attData['overtime_time']:''
                        ]);
                      }else{
                        DB::table('hr_buyer_att_template')->insert([
